const $ = document , _id = id => $.getElementById(id) , _qs = id => $.querySelector(id) , _qsa = id => $.querySelectorAll(id) , Bubble = new Audio('/Truth-or-Dare/Audio/bubble.wav') ,
Warn = Swal.mixin({
...
!function Security () {
    if (localStorage.getItem('Players') == null) {
        ...
        _qs('.error__wrapper h3').addEventListener('click' , () => {
            window.open('/Truth-or-Dare/' , '_top');
        });
...
function checkActive (elem) {
    ...
    if (!JSON.parse(btn.getAttribute('data-status'))) {
        ...
        icon.src = '/Truth-or-Dare/Pic/Icon/SVG/circle-check-regular.svg';
        icon.alt = '✔️';
    } else {
        wrapper.removeAttribute('class');
        btn.setAttribute('data-status' , JSON.stringify(false));
        icon.src = '/Truth-or-Dare/Pic/Icon/SVG/circle-xmark-regular.svg';
        icon.alt = '❌';
    };
};
